...
const AdminDashboard = () => {
  ...
  const stats = [
    ...
    { title: 'Avg. Price', value: `रू${toNPR(averagePrice)}`, icon: DollarSign, color: 'bg-orange-500' },
  ];
    ...
    const fetchAnalytics = async () => {
      ...
      try {
        ...
        const [salesRes, usersRes] = await Promise.all([
          fetch(`${API_URL}/admin/analytics/sales`, { headers: token ? { 'Authorization': `Bearer ${token}` } : {} }),
          fetch(`${API_URL}/admin/analytics/users`, { headers: token ? { 'Authorization': `Bearer ${token}` } : {} }),
        ]);
        const salesJson = await salesRes.json();
        const usersJson = await usersRes.json();
        // Merge sales and user growth data by month
        const merged = salesJson.map((s: any) => {
          const userEntry = usersJson.find((u: any) => u.month === s.month);
          return { ...s, users: userEntry ? userEntry.users : 0 };
        });
        setSalesData(merged);
      } catch (err) {
        setSalesData([]);
                ...
                {orders.slice(0, 5).map((order, i) => (
                  <div key={order._id || i} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                    ...
                    <div className="text-right">
                      <p className="font-medium">रू{toNPR(order.totalAmount || order.total)}</p>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {products.slice(0, 5).map((product, i) => (
                  <tr key={i}>
                    ...
                    <td className="px-4 py-2 whitespace-nowrap">रू{toNPR(product.price)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
